...
const startIndex = file.indexOf("{typeof document !== 'undefined' ? createPortal(");
const endIndex = file.indexOf("document.body\n) : null}");

if (startIndex !== -1 && endIndex !== -1) {
  const before = file.substring(0, startIndex + "{typeof document !== 'undefined' ? createPortal(".length);
...
`;
  ...
  fs.writeFileSync('src/components/dashboard/PatientRotinaView.tsx', before + newMiddle + after);
}
